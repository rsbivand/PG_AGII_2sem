...
levs |> as.data.frame() |> subset(name=="Poziom Województw", select=id, drop=TRUE) -> wlev 
levs |> as.data.frame() |> subset(name=="Poziom Gmin", select=id, drop=TRUE) -> glev 
```

Given the voivodeship level, we can find the 12-character identifier for the Pomeranian voivodeship; BDL uses 12 characters rather tha 7 used in TERYT above, simply interjecting identifiers for other level units:

```{r bdl3, size="footnotesize"}
(units_lev <- get_units(level=wlev, lang="en"))
units_lev |> as.data.frame() |> subset(name=="POMORSKIE", select=id, drop=TRUE) -> pom_id
```
