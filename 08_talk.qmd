...
The value of the single spatial parameter is found by line search (univariate optimisation). In `spautolm`, the objective function includes the use of numerical inversion of the ${\mathbf X}'{\mathbf A}{\mathbf X}$ matrix, where in the CAR case, ${\mathbf A} = {\mathbf I} - \rho{\mathbf W}$, but in the SAR case, it is ${\mathbf A} = ({\mathbf I} - \rho{\mathbf W})'({\mathbf I} - \rho{\mathbf W})$, using plug-in functions. Using `spautolm`, we get:
...
$$
\mathrm{LR} = -2 ({\mathcal l}(\theta_0) - {\mathcal l}(\theta_1))
$$

where ${\mathcal l}(\theta_0)$ is the log-likelihood of the nested model, and ${\mathcal l}(\theta_1)$ of the encompassing model, and where the number of degrees of freedom for $\chi^2$ is the difference in the estimated parameter counts.
...
The Rao's score (Lagrange Multiplier) test for residual spatial autocorrelation in the SDM model shows that the model is mis-specified.
...
$$
\mathrm{AIC} = 2 k - 2 {\mathcal l}(\theta)
$$
...
$$
\mathrm{BIC} = k \ln(n) - 2 {\mathcal l}(\theta)
$$
