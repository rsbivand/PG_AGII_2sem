...
form <- log(realNetPre) ~ log(units) + house +
 log(dens) + Metrop + log(realWgPre)
lm_obj <- lm(form, data=eng324)
...
library(spatialreg)
SLX_obj <- lmSLX(form, data=eng324, listw=lw,
 Durbin=update(form, ~ . - Metrop))
...
form_maj <- update(form, . ~. + Majority)
lm_obj_maj <- lm(form_maj, data=eng324)
summary(lm.RStests(lm_obj_maj, listw=lw, test="all"))
```
...
summary(SD.RStests(lm_obj_maj, listw=lw, test="SDM"),
 Durbin=update(form, ~ . - Metrop - Majority))
```
...
summary(SD.RStests(lm_obj_maj, listw=lw, test="SDEM"),
 Durbin=update(form, ~ . - Metrop - Majority))
```
...
library(spatialreg)
SLX_obj_maj <- lmSLX(form_maj, data=eng324, listw=lw,
 Durbin=update(form, ~ . - Metrop - Majority))
summary(lm.RStests(SLX_obj_maj, listw=lw, test="all"))
```


```{r echo=FALSE}
save(eng324, unb, orignb, file = paste0(io_dir, "/talk7.RData"))
```
